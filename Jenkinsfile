...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 4393
        NAMESPACE = "lumosapp-net"
        REGISTRY_HOSTNAME = "viktoriastgn"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "lumosapp-frontend"
        DEPLOYMENT_NAME = "lumosapp-frontend-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'lumosapp-net') {
